...
Model.getFieldNames = function() {
    return new Proxy(this.rawAttributes, {
        get: function(rawAttrs, name) {
            let fld = rawAttrs[name];
            if (fld) {
 ...
 */
function defineFpBook(db, tableName) {
 ...
 * @param { Database } db 
 */
function defineFpHistoryBook(db) {
    return db.define('2019年度扶贫办民政残联历史数据', {
        no: {
            field: '序号',
            type:  Database.INTEGER,
            primaryKey: true,
...
module.exports = {
    Database, createFpDb, defineFpTable, defineFpBook, defineFpHistoryBook, defineJbTable, Model
}